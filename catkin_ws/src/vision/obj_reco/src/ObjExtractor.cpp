...
std::vector<DetectedObject> ObjExtractor::GetObjectsInHorizontalPlanes(cv::Mat pointCloud)
{
	...
	for(int i=0; i<(int)objIdxClusters.size(); i++)
	{
		...
		if( detObj.shadowOriBoundBoxt2D.size.width <0.01 || detObj.shadowOriBoundBoxt2D.size.width > 0.35 )
			continue; 
		if( detObj.shadowOriBoundBoxt2D.size.height<0.01 || detObj.shadowOriBoundBoxt2D.size.height > 0.35 )
			continue; 
		if( detObj.height <0.01 || detObj.height > 0.35)
			continue;
